...
//
//   g++ -O2 -std=c++17 -o submission/codex_version/snaky_greed.exe submission/codex_version/snaky_greed.cpp
...
// Exact root-level escape check. The leaf playout is intentionally greedy and
// cheap; this DFS is used only for the four root candidates to reject moves
// that land in a one- or two-turn cul-de-sac behind our own trail.
static int survival_depth(int x, int y, int cap) {
    if (cap <= 0)
        return 0;
    int best = 0;
    for (int di = 0; di < 4; ++di) {
        Sim s = simulate(x, y, di);
        if (!s.alive || s.steps <= 0)
            continue;
        set_path(x, y, di, s.steps, true);
        int d = 1 + survival_depth(s.nx, s.ny, cap - 1);
...
static char decide(int mx, int my, int ex, int ey) {
    ...
    for (int di = 0; di < 4; ++di) {
        ...
        int surv = survival_depth(s.nx, s.ny, 8);
