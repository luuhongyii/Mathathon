...
array<double, 101> kernelDist(const vector<int>& h, double sigma) {
    ...
    int n = (int)h.size();
    for (int k = 0; k < n; ++k) {
        double recw = pow(0.92, n - 1 - k);  // exponential recency decay
        int v = h[k];
        for (int b = 1; b <= 100; ++b) {
            double dd = b - v;
...
array<double, 101> oppDist(int i, int oppPos) {
    ...
    if (n == 0) {
        ...
        // Spread of the opponent's recent bids.
        int start = max(0, n - 12);
        double mean = 0.0;
        int c = 0;
...
int chooseBid(const array<int, 4>& pos, const array<int, 4>& cum) {
    ...
    if (myDist >= 1 && myDist <= 100) {
        // I can finish this round. Finishing now dominates finishing later, so
        // pick the finishing bid most likely to succeed.
        int bFin = myDist;
        double psFin = -1.0;
        ...
        // An opponent that could finish within ~2 rounds is an imminent threat.
        bool oppClose = false;
        for (const OppInfo& o : opps)
            if (o.dist <= 160) oppClose = true;

        if (psFin >= 0.60 || oppClose) return bFin;

        // Finishing is risky and every opponent is far: spend one round
        // advancing safely so next round's finish is much safer.
        int bSet = 1;
        double bestEV = -1.0;
        ...
        if (pSafe(bSet) >= 0.75) return bSet;
        return bFin;
    }
    ...
    fprintf(stderr, "| cdf@80:");
    for (const OppInfo& o : opps) fprintf(stderr, " %.2f", o.cdf[80]);
    fprintf(stderr, "\n");
#endif
    return best;
}
